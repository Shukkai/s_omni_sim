...
def write_dense(path, setup):
    ...
    out = ["# Omni-LUT — dense baseline",
           "",
           "*All tables, no commentary. Generated by "
           "`analysis/memory/ram_model_run.py`; the reasoning behind each one "
           "is in `analysis/memory/ram_model_report.md` and `study.md` §22.*",
           ""]
    for line in setup:
        out.append(f"- {line}")
    out.append("")
