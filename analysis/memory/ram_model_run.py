...
FINDINGS = [
    "**Prefill is compute-bound and decode is DRAM-bound, at every context.** "
    "Decode spends 6.8× longer on DRAM than compute at 2K, narrowing to 1.1× "
    ...
    "utilisation falls with it. The 8 weight banks are there for capacity, "
    "not bandwidth.",

    "**Decode moves per token almost exactly what prefill moves in total** "
    "(2.65 GB vs 2.65 GB at 2K) — 2.58 GB of weights are re-read every step "
    ...
    "one column tile of accumulators.",
]
