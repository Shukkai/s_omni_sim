...
def get_model_configs(model_names=None):
    ...
    Args:
        model_names: List of model names to include.  If None, uses MODEL_NAMES.
...
# DRAM bandwidth assumption for roofline model
...
def run_single_simulation(hw_config: HardwareConfig, model_config: ModelConfig,
    ...
    Returns both raw total_cycles and roofline_total_cycles (effective cycles
    accounting for DRAM bandwidth bottleneck via the roofline model).
    """
    ...
    # Roofline: total time (seconds) considering memory bandwidth
    ttft, tpot = simulator.compute_roofline_latency(
        results, workload, dram_bandwidth_gbps=DRAM_BANDWIDTH_GBPS)
    roofline_total_time = ttft + tpot * max(1, workload.output_tokens - 1)
    # Convert roofline time back to "effective cycles" for consistent TOPS/mm² calc
    freq = hw_config.freq_mhz * 1e6
    roofline_total_cycles = roofline_total_time * freq

    metrics = {
        'total_cycles': total.cycles,
        'roofline_total_cycles': roofline_total_cycles,
    }
...
def run_all_simulations(output_json='hw_comparison_results.json', output_csv='hw_comparison_results.csv',
    ...
    if vary_input:
        ...
        varying_tokens = [128, 1024, 8192]
        fixed_tokens = fixed_output_tokens if fixed_output_tokens else 512
        token_type = 'input'
    else:
        # Vary output tokens, fix input tokens
        varying_tokens = [256, 512, 1024, 2048]
...
def save_to_csv(results, output_csv):
    ...
    with open(output_csv, 'w', newline='') as csvfile:
        ...
        writer.writerow(['Model', 'Hardware', 'Input Tokens', 'Output Tokens',
                        'Total Cycles', 'Roofline Total Cycles'])
...
def print_summary(results):
    ...
    for model_name in sorted(results.keys()):
        ...
        for key in sorted_keys:
            ...
            print(f"  {'Hardware':<15} {'Total Cycles':<20} {'Roofline Cycles':<20}")
            print("  " + "-" * 55)
            
            for hw_name in sorted(hw_results.keys()):
                metrics = hw_results[hw_name][key]
                print(f"  {hw_name:<15} {metrics['total_cycles']:<20,} "
                      f"{metrics['roofline_total_cycles']:<20,.0f}")
...
def main():
    ...
    if vary_input:
        output_json = 'hw_tops-mm2.json'
        output_csv = 'hw_tops-mm2.csv'
    ...
    # --- Split into two JSON files: raw cycles vs roofline cycles ---
    raw_results = {}    # total_cycles only (no roofline)
    roof_results = {}   # roofline_total_cycles (renamed to total_cycles for plot compat)

    for model_name, hw_dict in results.items():
        raw_results[model_name] = {}
        roof_results[model_name] = {}
        for hw_name, token_dict in hw_dict.items():
            raw_results[model_name][hw_name] = {}
            roof_results[model_name][hw_name] = {}
            for key, metrics in token_dict.items():
                base = {
                    'input_tokens': metrics['input_tokens'],
                    'output_tokens': metrics['output_tokens'],
                }
                raw_results[model_name][hw_name][key] = {
                    **base, 'total_cycles': metrics['total_cycles']}
                roof_results[model_name][hw_name][key] = {
                    **base, 'total_cycles': metrics['roofline_total_cycles']}

    raw_json = 'hw_tops-mm2.json'
    roof_json = 'hw_tops-mm2_roofline.json'
    with open(raw_json, 'w') as f:
        json.dump(sort_results(raw_results), f, indent=2)
    with open(roof_json, 'w') as f:
        json.dump(sort_results(roof_results), f, indent=2)

    print(f"\nRaw-cycle results saved to:      {raw_json}")
    print(f"Roofline-cycle results saved to: {roof_json}")
    print("\n" + "=" * 80)
    print("Done! Both files use the same 'total_cycles' key for plot compatibility.")
    print("=" * 80)
